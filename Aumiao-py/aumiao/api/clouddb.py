...
class CoconutCloudAdmin:
	...
	def get_dict_entries(self, dict_id: int, work_id: int, offset: int = 1, limit: int = 500) -> dict:
		"""
		获取云字典条目列表(分页)
		Args:
			dict_id: 字典 ID
	...
	def migrate_dict(self, db_id: str, from_env: int = 2, to_env: int = 1) -> dict:
		"""
		迁移云字典环境
		Args:
			db_id: 数据库 ID
		...
		Returns:
			迁移结果
		"""
		data = {"db_id": db_id, "from_env": from_env, "to_env": to_env}
		response = self._client.send_request(
